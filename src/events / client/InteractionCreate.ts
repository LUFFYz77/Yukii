...
import { Context, Event, type Client } from "../../structures/index";
    ...
    if (
      ...
      if (
        ...
        const ctx = new Context(
          chatInteraction as any,
          chatInteraction.options.data as any
        );
        ctx.setArgs(chatInteraction.options.data as any);
        ...
        if (command.permissions) {
          ...
          if (command.permissions?.dev && this.client.env.OWNER_IDS) {
            const isDev = this.client.env.OWNER_IDS.includes(
              interaction.user.id
            );
            if (!isDev) return;
          }
