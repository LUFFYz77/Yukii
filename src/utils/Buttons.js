...
function getButtons(player, client) {
    var buttonData = [
        ...
        {
            customId: 'PAUSE_BUT',
            emoji: (player === null || player === void 0 ? void 0 : player.paused) ? client.emoji.resume : client.emoji.pause,
            style: (player === null || player === void 0 ? void 0 : player.paused) ? discord_js_1.ButtonStyle.Success : discord_js_1.ButtonStyle.Secondary,
        },
        {
    ...
    return buttonData.reduce(function (rows, _a, index) {
        var customId = _a.customId, emoji = _a.emoji, style = _a.style;
        if (index % 5 === 0)
            rows.push(new discord_js_1.ActionRowBuilder());
        var emojiFormat;
        if (typeof emoji === 'string' && emoji.startsWith('<:')) {
            var match = emoji.match(/^<:\w+:(\d+)>$/);
            emojiFormat = match ? match[1] : emoji;
        }
        else {
            emojiFormat = emoji;
        }
        var button = new discord_js_1.ButtonBuilder().setCustomId(customId).setEmoji(emojiFormat).setStyle(style);
