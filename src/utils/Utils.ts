...
export class Utils {
	...
	public static updateStatus(client: Client, guildId?: string): void {
		const { user } = client;
	...
	public static emojiToImage(emoji: string): any | null {
	...
	public static async paginate(client: Client, ctx: Context, embed: any[]): Promise<void> {
		if (embed.length < 2) {
