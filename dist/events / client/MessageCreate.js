    ...
    async run(message) {
        ...
        if (mention.test(message.content)) {
            const embed = this.client
                ...
                .setDescription(`My Name is ${this.client.user?.displayName}.\n` +
                `My prefix for this server is **\`${guild?.prefix}\`**.\n\n` +
                `Do you need help? please use **\`${guild?.prefix}help\`**!!!`)
                .setFooter({
        ...
        const prefixRegex = new RegExp(`^(<@!?${this.client.user?.id}>|${escapeRegex(guild.prefix)})\\s*`, "i");
