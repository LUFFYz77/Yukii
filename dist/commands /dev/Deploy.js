...
class Deploy extends index_1.Command {
    ...
    async run(client, ctx, _args) {
        const row = new discord_js_1.ActionRowBuilder().addComponents(new discord_js_1.ButtonBuilder().setCustomId('deploy-global').setLabel('Global').setStyle(discord_js_1.ButtonStyle.Secondary), new discord_js_1.ButtonBuilder().setCustomId('deploy-guild').setLabel('Guild').setStyle(discord_js_1.ButtonStyle.Secondary));
        let msg;
        try {
